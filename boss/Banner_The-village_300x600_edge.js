...
(function(symbolName){Symbol.bindElementAction(compId,symbolName,"${SonOff}","click",function(sym,e){sym.$("SonOff").hide();sym.$("SonOn").show();sym.$("video")[0].muted=false;});
//Edge binding end
...
(function(symbolName){Symbol.bindTriggerAction(compId,symbolName,"Default Timeline",250,function(sym,e){Enabler.loadModule(studio.module.ModuleId.RAD_VIDEO,function(){var VIDEO_COMPONENT='videoComp';var stage=sym.getComposition().getStage();var videoComp=stage.getVariable(VIDEO_COMPONENT);if(!videoComp){videoComp=new studio.sdk.rad.Video({id:'Edge Video',autoplay:false,controls:false,sources:['https://res.cloudinary.com/rciparis/video/upload/v1475228432/wekrop/Production/6850/Hugo_Boss_Scent_Her_15s_300x270.mp4']});videoComp.setElement(sym.getSymbolElementNode());stage.setVariable(VIDEO_COMPONENT,videoComp);}else{videoComp.getVideoElement().play();}});});
//Edge binding end
})("video-centerer");
